...
AddEventHandler('onResourceStart', function(resource)
    ...
    -- Check if players table exists. If not, auto-import qbx_core.sql
    local playersTableExists = MySQL.scalar.await("SELECT COUNT(*) FROM information_schema.TABLES WHERE TABLE_NAME = 'players' AND TABLE_SCHEMA = DATABASE()")
    if not playersTableExists or playersTableExists == 0 then
        local sqlFile = LoadResourceFile(cache.resource, 'qbx_core.sql')
        if sqlFile then
            lib.print.info('Importing qbx_core.sql database schema automatically...')
            for statement in string.gmatch(sqlFile, "[^;]+") do
                local cleanQuery = statement:gsub("^%s*", ""):gsub("%s*$", "")
                if cleanQuery ~= "" then
                    local success, err = pcall(function()
                        MySQL.query.await(cleanQuery)
                    end)
                    if not success then
                        lib.print.error(('Failed to execute auto-import query: %s\nError: %s'):format(cleanQuery, tostring(err)))
                    end
                end
            end
            lib.print.info('Auto-import of qbx_core.sql completed.')
        else
            lib.print.error('qbx_core.sql file not found in resource, cannot auto-import database schema!')
        end
    end
